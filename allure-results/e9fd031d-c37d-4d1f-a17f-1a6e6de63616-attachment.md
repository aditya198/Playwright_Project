...
```ts
  ...
  14 |        await login.HRMLogin(process.env.Base_User!, process.env.Base_Pass!)
  15 |     });
  16 |     test('Add Emplyee successfully', async ({page})=>{
  17 |         await emp.AddEmp('Aditya','A','Batra')     
  18 |     })
> 19 |     test.afterEach(async() =>{
     |          ^ Test timeout of 30000ms exceeded while running "afterEach" hook.
  20 |         await login.HRMLogout()
  21 |     })
  22 | })
```